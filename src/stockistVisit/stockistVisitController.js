...
const confirmStockistVisit = async (req, res) => {
  try {
    ...
    // Check if distance verification is enabled and stockist's location is available
    if (enableDistanceVerification && stockist.latitude && stockist.longitude) {
      // Calculate distance
      ...
      if (distance > maxDistanceMeters) {
        return res.status(200).json({
          status: false,
          success: false,
          message: `You are ${Math.round(distance)} meters away from the stockist's location. Please be within ${maxDistanceMeters} meters to confirm the visit.`,
          distance: Math.round(distance),
          allowedDistance: maxDistanceMeters
        });
      }
    } else if (!enableDistanceVerification) {
      console.log(`Distance verification is disabled by admin setting. Allowing stockist visit confirmation without geo-fencing.`);
    } else {
...
const bulkConfirmStockistVisits = async (req, res) => {
  ...
  try {
    ...
    for (const item of itemsToProcess) {
      ...
      const stockist = visit.Stockist;
      const { enableDistanceVerification, maxDistanceMeters } = await getActiveVisitDistanceConfig(sequelize);
      if (enableDistanceVerification && stockist && stockist.latitude && stockist.longitude && userLatitude && userLongitude) {
        const distance = getDistance(
          userLatitude,
          userLongitude,
          stockist.latitude,
        ...
        if (distance > maxDistanceMeters) {
          errors.push({
            id: visitId,
            message: `You are ${Math.round(distance)} meters away from the stockist's location. Please be within ${maxDistanceMeters} meters.`
          });
          continue;
        }
      }
...
// BULK CREATE / SCHEDULE stockist visits
const bulkCreateStockistVisits = async (req, res) => {
  const sequelize = req.app.get('sequelize');
  ...
  try {
    ...
    for (let i = 0; i < visits.length; i++) {
      const item = visits[i];
      const stockistId = item.stockist_id || item.stockistId;
      ...
      if (!stockistId) {
        errors.push({ index: i, item, error: 'Missing stockist_id' });
        continue;
      }
      if (!targetUserId) {
      ...
      const stockist = await Stockist.findByPk(stockistId, { transaction });
      if (!stockist) {
        errors.push({ index: i, item, error: `Stockist with ID '${stockistId}' not found` });
        continue;
      }
      ...
      // Check if an unconfirmed visit already exists on the same date for this stockist and user
      const existingUnconfirmed = await StockistVisit.findOne({
        where: {
      ...
      if (existingUnconfirmed) {
        skippedVisits.push({
          index: i,
          item,
          reason: 'An unconfirmed visit for this stockist on this date already exists for this user',
          existingVisitId: existingUnconfirmed.id
        });
        continue;
      }

      const newVisit = await StockistVisit.create({
        stockist_id: stockistId,
        user_id: targetUserId,
        date: visitDate,
        notes
    ...
    return res.status(201).json({
      success: true,
      message: `Bulk stockist schedule processed: ${createdVisits.length} created, ${skippedVisits.length} skipped, ${errors.length} errors.`,
      summary: {
        total: visits.length,
    ...
    console.error('Error in bulkCreateStockistVisits:', error);
    return res.status(500).json({
      success: false,
      message: error.message || 'Server error during bulk stockist visit creation'
    });
  }
};
